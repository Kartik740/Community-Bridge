...
const NeedsHeatmap = ({ coordinates }) => {
  ...
  const getColor = (point) => {
    ...
    if (point.answers) {
      const urgencyAnswer = point.answers.find(a => 
         typeof a.value === 'string' && ['Critical', 'High', 'Moderate', 'Low'].includes(a.value)
      );
      if (urgencyAnswer) urgencyValue = urgencyAnswer.value;
    }
    
    switch (urgencyValue.toLowerCase()) {
  ...
  return (
    <div className="w-full h-96 rounded-3xl overflow-hidden glass-panel border border-white/60 shadow-inner relative z-0">
      <MapContainer center={center} zoom={5} style={{ width: '100%', height: '100%', zIndex: 1 }}>
        <TileLayer
